...
class CorDslTest {
    ...
    fun `should throw when exception and no except`() = runTest {
        val chain = rootChain<TestContext> {
            worker("throw", "Throws error") { throw RuntimeException("some error") }
        }
        assertFails {
            execute(chain)
        }
    ...
    fun `complex chain example`() = runTest {
        val chain = rootChain<TestContext> {
            worker {
                ...
                description = "At chain processing start, status is not yet set. Check it"
            ...
            chain {
                ...
                worker(
                    ...
                    description = "Example of handler as lambda"
                ) {
                    some += 4
                }
            }
